...
const Login = ({ isLogin, setIsLogin }) => {
  ...
  const {
    ...
    error, // Error message from login attempt
    sucess, // Success message after login
    formData, // Current form data (email, password, role)
  } = useLogin();
  ...
  return (
    ...
    >
      {/*} Login card container with shadow and border*/}
      <div
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <button
            type="submit"
            className={`w-full h-11 rounded-lg font-semibold transition-all duration-300 active:scale-95
            ${
              isDark
        ...
        {/* Visual divider between form and OAuth login */}
        ...
        <button
          onClick={handleLogin}
          className={`w-full flex items-center justify-center gap-2 py-3 px-2 rounded-lg border font-medium transition-all duration-300
          ${
            isDark
        ...
        >
          ...
          {/* Clickable link to toggle between login/register */}
          <span
            onClick={() => setIsLogin(!isLogin)}
            className="text-blue-500 cursor-pointer ml-2 hover:text-blue-600 font-semibold"
          >
